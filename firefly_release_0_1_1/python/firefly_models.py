...
class StellarPopulationModel:
	...
	def __init__(self, specObs, outputFile, cosmo, models = 'm11', model_libs = ['MILES_UVextended'], imfs = ['ss','kr'], hpf_mode = 'on', age_limits = [6,10.1], downgrade_models = True, dust_law = 'calzetti', max_ebv = 1.5, num_dust_vals = 200, dust_smoothing_length = 200, max_iterations = 10, fit_per_iteration_cap = 1000, pdf_sampling = 300, data_wave_medium = 'vacuum', Z_limits = [0.001,10], wave_limits = [0,99999990], suffix = "",use_downgraded_models = False, write_results=True, flux_units=10**(-17)):
		...
		# add in to loop bellow Mastar fwhm value but convert it from spectral res of 1800 first
	...
	def get_model(self, model_used, imf_used, deltal, vdisp, wave_instrument, r_instrument, ebv_mw):
		...
		if self.models == 'm11':
			...
			age_idx =np.where(t==age)[0][0]
			slope_idx = np.where(s==slope)[0][0]
			z_idx = np.where(Z==z)[0][0]
			'''
			slope = 1.8
			slope_idx = np.where(s==slope)[0][0]

			wavelength = wave
			flux, model_flux, age_model, metal_model = [], [],[],[]
			i,j=0,0

			for i in range(len(t)):
				for j in range(len(Z)):
					age_model.append(t[i])
					metal_model.append(Z[j])
					flux = fluxgrid[i,j,slope_idx,:]
